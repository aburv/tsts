...
class LoginServices:
    ...
    def login(self, user_data: dict, login_data: dict) -> dict:
        ...
        user_id_services = UserIdServices()
        services = UserServices()
        u_id = user_id_services.get_user_id_by_id_value(user_id_value)

        if u_id is None:
            u_id = services.create_user(user_data)
            pic_url = user_data.get("picUrl", "")
            if pic_url != "" and user_data.get("dp", "") == "":
                i_id = ImageServices().load_and_save(pic_url, u_id)
                services.update_user({"id": u_id, "dp": i_id})
        else:
            user = services.get_user_by_id(u_id)
            if not user:
