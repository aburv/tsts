...
class LoginServiceTest(unittest.TestCase):
    ...
    def test_should_create_new_user_and_login_with_location_and_pic_url_on_login(self,
                                                                                 mock_login,
                                                                                 mock_auth_service,
                                                                                 mock_image_services,
                                                                                 mock_load_and_save,
        ...
        mock_location_services.assert_called_once_with()
        mock_create_location.assert_called_once_with({'name': 'name'}, 'user_id')
        mock_image_services.assert_called_once_with()
        mock_load_and_save.assert_called_once_with('url', 'user_id')
        mock_update_user.assert_called_once_with({'id': 'user_id', 'dp': 'i_id'})
        mock_data.on_data.assert_called_once_with(
            {'location': {'name': 'name'}, 'userId': 'user_id', 'locationId': 'location_id'})
    ...
    def test_should_create_login_with_existing_user_without_location_on_login(self,
        ...
        # mock_user_ids_by_user.assert_called_once_with('user_id')
